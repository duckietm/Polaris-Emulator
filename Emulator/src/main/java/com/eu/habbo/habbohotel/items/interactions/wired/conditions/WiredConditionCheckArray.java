...
public final class WiredConditionCheckArray extends InteractionWiredCondition implements WiredLargePayload {
    ...
    private boolean evaluateMatch(
        ...
        if (this.searchScope == SCOPE_SPECIFIC_INDEX) {
            ...
            return entry != null
                    && WiredArrayEditorSupport.matchesEntry(entry, resolved, this.criteriaMode == CRITERIA_ANY);
        }
        ...
        for (WiredArrayEntry entry : value.entriesView().values()) {
            if (WiredArrayEditorSupport.matchesEntry(entry, resolved, this.criteriaMode == CRITERIA_ANY)) matches++;
        }
    ...
    private boolean evaluateState(WiredContext ctx, WiredArrayRuntimeSupport.Owner owner, WiredArrayView value) {
    ...
    public boolean saveData(WiredSettings settings) {
        ...
        try {
            ...
            if (nextMode == MODE_MATCH && !WiredArrayEditorSupport.validRawCriteria(data.criteria)) return false;
            boolean rawIndexValid = WiredArrayEditorSupport.validRawAddress(data.index);
            boolean rawResultReferenceValid = WiredArrayEditorSupport.validRawReference(data.resultReference);
            boolean rawStateReferenceValid = WiredArrayEditorSupport.validRawReference(data.stateReference);
            ...
            WiredArrayReference nextResultReference = WiredArrayEditorSupport.normalizeReference(data.resultReference);
            WiredArrayReference nextStateReference = WiredArrayEditorSupport.normalizeReference(data.stateReference);
            ...
            if (nextMode == MODE_MATCH) {
                ...
                for (WiredArrayCriterion criterion : nextCriteria) {
                    if (definition.getArrayDefinition().getField(criterion.fieldId) == null
                            ...
                            || !WiredArrayEditorSupport.isValidReference(criterion.reference, room)) return false;
                }
                if (nextScope == SCOPE_SPECIFIC_INDEX) {
                    if (!rawIndexValid || !WiredArrayEditorSupport.isValidAddress(nextIndex, definition, room)) {
                        return false;
                    }
                } else if (resultNeedsReference(nextResultMode)
                        && (!rawResultReferenceValid
                                || !WiredArrayEditorSupport.isValidReference(nextResultReference, room))) {
                    return false;
                }
            } else if ((nextStateCheck == STATE_LENGTH || nextStateCheck == STATE_AVAILABLE_INDEXES)
                    && (!rawStateReferenceValid
                            || !WiredArrayEditorSupport.isValidReference(nextStateReference, room))) {
                return false;
            }

            List<HabboItem> nextItems = WiredArrayEditorSupport.parseItems(settings.getFurniIds(), room);
            ...
            this.stateComparison = WiredArrayEditorSupport.normalizeComparison(params[8]);
            ...
            return true;
        } catch (WiredSaveException | RuntimeException exception) {
    ...
    public void loadWiredData(ResultSet set, Room room) throws SQLException {
        ...
        try {
            ...
            this.stateComparison = WiredArrayEditorSupport.normalizeComparison(data.stateComparison);
            ...
            this.resultReference = WiredArrayEditorSupport.normalizeReference(data.resultReference);
            this.stateReference = WiredArrayEditorSupport.normalizeReference(data.stateReference);
    ...
    public void serializeWiredData(ServerMessage message, Room room) {
        ...
        message.appendInt(2); // Reserved legacy comparison slot; resultMode carries the operation.
    ...
    public boolean requiresTriggeringUser() {
        ...
        return WiredArrayEditorSupport.requiresTrigger(this.index)
                || this.criteria.stream().anyMatch(value -> WiredArrayEditorSupport.requiresTrigger(value.reference))
                || WiredArrayEditorSupport.requiresTrigger(this.resultReference)
                || WiredArrayEditorSupport.requiresTrigger(this.stateReference);
    }
