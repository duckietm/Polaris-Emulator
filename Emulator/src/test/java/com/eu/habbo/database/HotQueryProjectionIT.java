...
class HotQueryProjectionIT {
    ...
    void pinnedHotQueriesExecuteAgainstTheRuntimeSchema() throws Exception {
        ...
        try (HikariDataSource dataSource =
                     TestDatabase.freshDatabase("hot_query_projection")) {
            MigrationRunner.migrate(dataSource);

            execute(dataSource, sql(
                    MarketPlace.class,
                    "PURCHASE_OFFER_SQL"), 0);
            execute(dataSource, sql(
                    MarketPlace.class,
                    "PURCHASE_ITEM_SQL"), 0);
            execute(dataSource, sql(
            ...
            execute(dataSource, sql(
                    RoomManager.class,
                    "PUBLIC_ROOMS_SQL"), "1", "1");
            execute(dataSource, sql(
                    RoomManager.class,
                    "ROOM_IDS_BY_OWNER_NAME_SQL"), "nobody");
            execute(dataSource, sql(
                    RoomManager.class,
                    "ROOMS_BY_OWNER_ID_SQL"), 0);
        }
    }
    ...
    private static String sql(Class<?> owner, String fieldName)
            throws Exception {
