...
- ✅⚠️ **HS3** (`ServerMessage.get()` `copy()` → `retainedDuplicate()`) — done (4.2.65). Verified safe: the buffer is `Unpooled.buffer()` (GC-backed, no pool to corrupt); `retainedDuplicate()` is +1 refcount and the encoder `release()`s it after `writeBytes`, so the original stays at refcount 1 (same lifecycle as `copy()`, GC'd). The 4-byte length prefix is now finalized **once** via double-checked locking, removing the per-`get()` concurrent write to the shared buffer. Compiles + 464 tests green incl. PacketSerializationCharacterizationTest. **⚠️ The concurrent broadcast/refcount path is single-threaded in the tests — smoke-test in a busy room before fully trusting; trivially revertable (one commit).**
